...
fn test_detail_format() {
    ...
    let output = run_code_stats(&[project_root.to_str().unwrap(), "--format", "detail"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_json_format() {
    ...
    let output = run_code_stats(&[project_root.to_str().unwrap(), "--format", "json"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_json_format_with_empty_directory() {
    ...
    let output = run_code_stats(&[temp_dir.path().to_str().unwrap(), "--format", "json"]);
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_format_case_insensitive() {
    ...
    for format in &formats {
        let output = run_code_stats(&[project_root.to_str().unwrap(), "--format", format]);
